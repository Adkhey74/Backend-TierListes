...
export interface UpdateTierListItemInput {
  id?: string;
  logoId: string;
  category: string;
}

export interface UpdateTierListCommand {
  userId: string;
  id: string;
  title: string;
  items?: UpdateTierListItemInput[];
}

@Injectable()
export class UpdateTierListUseCase {
  constructor(
  ...
  async execute(command: UpdateTierListCommand): Promise<TierList> {
    const tierList = await this.tierListRepository.update(
      command.id,
      command.userId,
      {
        title: command.title,
    ...
    if (items.length > 0) {
      ...
      await this.tierListRepository.deleteItems(tierList.id);
